...
class Mydoliboard extends CommonObject
{
	...
	function fetch($rowid)
	{
		...
		$sql.= " ,blocAmode, blocBmode, blocCmode, blocDmode ,blocAtitle, blocBtitle, blocCtitle, blocDtitle,blocAstack, blocBstack, blocCstack, blocDstack";
		...
		if ($resql)
		{
			if ($this->db->num_rows($resql) > 0)
			{
				...
				$this->blocAstack	= $res['blocAstack'];
				$this->blocBstack	= $res['blocBstack'];
				$this->blocCstack	= $res['blocCstack'];
				$this->blocDstack	= $res['blocDstack'];
	...
	function create($user='')
	{
		...
		$sql.= ", ".(! isset($this->blocAstack)?'NULL':$this->blocAstack);
		$sql.= ", ".(! isset($this->blocBstack)?'NULL':$this->blocBstack);
		$sql.= ", ".(! isset($this->blocCstack)?'NULL':$this->blocCstack);
		$sql.= ", ".(! isset($this->blocDstack)?'NULL':$this->blocDstack);
...
//print $sql;
	 ...
	 * 	@return		void
	 */
	function gengraph($bloc, $mode, $title, $dim=0, $width = 1150, $height = 300)
	{
		...
		$blocstack = 'bloc' . $bloc .'stack';
		$stack = $this->$blocstack;
		// récupération des requetes de ce bloc
		$sql = "SELECT * FROM ".MAIN_DB_PREFIX."mydoliboardsheet";
		$sql.= " WHERE fk_mdbpage = ".$this->rowid;
		$sql.= " AND displaycell = '".$bloc."'";
		...
		if ($resql)
		{
			...
			$sz.='<h2 align ="center">'.$descligne[0].'</h2>';
	...
	function update($user='')
	{
		...
		$sql .= ", blocAstack ='".$this->db->escape($this->blocAstack)."'";
		$sql .= ", blocBstack ='".$this->db->escape($this->blocBstack)."'";
		$sql .= ", blocCstack ='".$this->db->escape($this->blocCstack)."'";
		$sql .= ", blocDstack ='".$this->db->escape($this->blocDstack)."'";
...
//				break;
	...
	function fetch($rowid)
	{
		...
		$sql = "SELECT fk_mdbpage, titlesheet, description, displaycell, cellorder, perms, langs, author, active, querymaj, querydisp, graphtype, ordre";
		$sql.= " FROM ".MAIN_DB_PREFIX."mydoliboardsheet";
		$sql.= " WHERE rowid = '".$rowid."'";
	...
	function create($user='')
	{
		...
		$this->ordre=(!is_array($this->ordre)?trim($this->ordre):'');
	...
	function update($user='')
	{
		...
		$sql .= ", ordre ='".$this->db->escape($this->ordre)."'";
		$sql .= ", author ='".$this->db->escape($this->author)."'";
		$sql .= ", active =".$this->db->escape($this->active);
		$sql .= " WHERE rowid = ".$this->rowid;
		dol_syslog(get_class($this)."::update sql=".$sql);
